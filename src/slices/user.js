...
export const getUserAuth = createAsyncThunk("users/me", async (thunkAPI) => {
  try {
...
export const getAllUsers = createAsyncThunk(
  ...
  async (currentId, thunkAPI) => {
    try {
      const response = await usersService.getAllUsers(currentId);
      //thunkAPI.dispatch(setMessage(response.data.message));
      return response.data;
    } catch (error) {
      const message =
...
const usersSlice = createSlice({
  name: "users",
  initialState: { me: null, listUsers: null, users: null },
  extraReducers: {
